import numpy as np
from scipy.signal import hanning, spectrogram, resample
...
def get_envelope(audio, audio_fs, new_fs, cof=25):
    ...
    envelope_long = filtfilt(b, a, env_hilb, axis=0) #filtfilt makes it non-causal (fwd/backward)

    envelope = resample(envelope_long.T, np.floor(envelope_long.shape[1]/(audio_fs/new_fs)))
    return envelope
